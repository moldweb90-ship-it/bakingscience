...
def add(slug, title, desc, date, cat, emoji, readTime, keywords, content, relatedIng, relatedPosts):
    posts.append({
        "slug": slug, "title": title, "description": desc, "date": date,
        "category": cat, "emoji": emoji, "readTime": readTime,
        "keywords": keywords, "content": content,
        "relatedIngredients": relatedIng, "relatedPosts": relatedPosts
    })
...
flours = [
    ...
    {"id": "bread-flour", "name": "Bread Flour", "density": 0.550,
     ...
     "storage": "Bread flour has slightly more oil than AP flour due to higher protein. Store in the fridge for longest shelf life - up to a year.",
...
for i, f in enumerate(flours):
    ...
    if not f["recipes"]:
        c100 = round(100 / (f["density"] * 236.588), 2)
        c200 = round(200 / (f["density"] * 236.588), 2)
        c250 = round(250 / (f["density"] * 236.588), 2)
        recipes_html = f'\n<p>Common amounts: 100g = {c100} cups, 200g = {c200} cups, 250g = {c250} cups. Try our <a href="/{f["id"]}/">converter</a> for any weight.</p>'
...
<p>Convert any weight with our <a href="/{f["id"]}/">{f["name"].lower()} grams to cups converter</a>.</p>"""
    ...
    add(f"{f['id']}-grams-to-cups-guide",
        f"How to Convert {f['name']} from Grams to Cups (Complete Guide)",
        f"Complete guide to converting {f['name'].lower()} from grams to cups. {f['cup_grams']}g per cup (spoon & level), plus dip & sweep and sifted methods. Free converter.",
        date, "ingredients", "\U0001f33e", 5,
        [f"{f['id'].replace('-', ' ')} grams to cups", f"{f['id'].replace('-', ' ')} cup measurement", f"how much is 1 cup {f['id'].replace('-', ' ')}"],
        content, [f["id"]] + f["related"],
        ["why-cup-measurements-fail", "how-to-measure-flour-correctly", "what-is-spoon-and-level-method"])
...
for i, s in enumerate(sugars):
    ...
    if not s["recipes"]:
        c100 = round(100 / (s["density"] * 236.588), 2)
        c200 = round(200 / (s["density"] * 236.588), 2)
        recipes_html = f'\n<p>Common amounts: 100g = {c100} cups, 200g = {c200} cups. Try our <a href="/{s["id"]}/">converter</a> for any weight.</p>'

    method_note = f"For {s['name'].lower()}, the measurement method doesn't change the result since it's a liquid." if is_liquid else f"With dip and sweep (packed), it's about <strong>{s['cup_dip']}g</strong>. Sifted, it's about <strong>{s['cup_sifted']}g</strong>."
...
<p>One cup of {s["name"].lower()} weighs approximately <strong>{s["cup_grams"]}g</strong>. {method_note}</p>
...
<p>Convert any weight with our <a href="/{s['id']}/">{s["name"].lower()} grams to cups converter</a>.</p>"""
    ...
    add(f"{s['id']}-grams-to-cups-guide",
        f"How to Convert {s['name']} from Grams to Cups (Complete Guide)",
        f"Complete guide to converting {s['name'].lower()} from grams to cups. {s['cup_grams']}g per cup. Free converter for any weight.",
        date, "ingredients", "\U0001f36c", 4,
        [f"{s['id'].replace('-', ' ')} grams to cups", f"{s['id'].replace('-', ' ')} cup measurement", f"how much is 1 cup {s['id'].replace('-', ' ')}"],
        content, [s["id"]] + s["related"],
        ["why-cup-measurements-fail", "grams-vs-cups-which-is-better", "baking-measurement-mistakes"])
...
print(f"Added {len(posts)} articles (Articles 11-20: Ingredients)")
print(f"Total articles now: {10 + len(posts)}")
